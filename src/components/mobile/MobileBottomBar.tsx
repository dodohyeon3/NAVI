...
const DRAWING_ITEMS = [
  { value: 'trendline' as const, label: '추세선',   icon: '↗', desc: '저점·고점을 이어 추세 방향 표시' },
  { value: 'fibonacci' as const, label: '피보나치', icon: '𝚽', desc: '고점→저점 클릭 시 되돌림 레벨 표시' },
]
...
export function MobileBottomBar() {
  ...
  /* 튜토리얼 진행 중에는 bottom bar 숨김 (튜토리얼 자체 UI 사용) */
  ...
  return (
    <>
      {/* ── 작도 모드 활성 배너 ────────────────────────────── */}
      {isDrawing && (
        <div className="fixed bottom-14 left-0 right-0 z-30 px-3 pb-0.5"
          ...
          <div className="bg-amber-500/[0.12] border border-amber-500/30 rounded-xl
            ...
            <button
              ...
              className="text-[11px] text-navi-muted hover:text-navi-text
                         transition-colors px-2.5 py-1 rounded-lg border border-navi-border shrink-0"
            >
              취소
            </button>
          </div>
      ...
      <div
        className="fixed bottom-0 left-0 right-0 z-40
                   bg-navi-surface/96 backdrop-blur-md
                   border-t border-navi-border"
        style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}
      >
      ...
      >
        <div className="p-4 pb-8 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {ANALYSIS_ITEMS.map(({ slug, label, name }) => {
              ...
              return (
                ...
                >
                  <span className={clsx(
                    'text-[15px] font-bold w-10 text-center shrink-0 leading-none',
          ...
          {activeIndicators.size > 0 && (
            <p className="text-center text-[11px] text-navi-muted pt-1">
              {activeIndicators.size}개 활성화됨
            </p>
          )}
        </div>
      </MobileBottomSheet>
      ...
      >
        <div className="p-4 pb-8 space-y-2.5">
          <p className="text-[11px] text-navi-muted">차트에서 직접 그려봐요</p>
          ...
          {DRAWING_ITEMS.map(tool => {
            ...
            return (
              ...
              >
                <span className={clsx(
                  'text-[20px] w-8 text-center shrink-0',
                ...
                <div className="text-left flex-1">
                  ...
                  <p className="text-[12px] text-navi-muted mt-0.5">{tool.desc}</p>
                </div>
                {active && (
                  <span className="shrink-0 text-[10px] font-bold
                                   text-amber-400 bg-amber-400/15 px-2.5 py-1 rounded-full">
                    진행 중
                  </span>
                )}
              </button>
          ...
          >
            <span className="text-[20px] w-8 text-center shrink-0 text-navi-muted">✕</span>
            <div className="text-left">
              <p className="text-[14px] font-semibold text-navi-text">전체 삭제</p>
              <p className="text-[12px] text-navi-muted mt-0.5">그린 선을 모두 지워요</p>
            </div>
          </button>
        </div>
      </MobileBottomSheet>
        ...
        <div className="p-4 pb-10 space-y-5">
          ...
          <div>
            <p className="text-[10px] font-bold text-navi-muted uppercase
                          tracking-[0.08em] mb-2.5">기초 과정</p>
            <button
              onClick={() => { start(); close() }}
            ...
            <p className="text-center text-[11px] text-navi-muted mt-2">
              약 7~10분 · 16단계 실습 · 틀려도 괜찮아요
            </p>
          </div>
          ...
          <div>
            <p className="text-[10px] font-bold text-navi-muted uppercase
                          tracking-[0.08em] mb-2.5">추가 학습</p>
              ...
              {LESSON_ITEMS.map(({ key, label }) => (
                <button
                  ...
                  className="w-full flex items-center justify-between
                             px-4 py-3.5 rounded-2xl border border-navi-border
                             bg-navi-surface2 hover:bg-navi-surface3
                             transition-all active:scale-[0.97]"
                >
                  <span className="text-[13px] font-medium text-navi-text">
                    {label}
                  </span>
                  <span className="text-navi-muted text-[12px]">→</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <p className="text-[10px] font-bold text-navi-muted uppercase
                          tracking-[0.08em] mb-2.5">실전 챌린지</p>
            <Link
              href="/simulate?guide=1"
              onClick={close}
              className="block w-full py-4 rounded-2xl text-[13px] font-semibold
                         text-center border-2 border-navi-border2 text-navi-text
