...
interface FibSwing {
  ...
  bouncePrice: number
  fibKey:      FibKey
  focusBars:   number   // n - loIdx + margin (데이터 끝~저점 앞까지 모두 표시)
}
...
function findFibSwing(data: CandleData[]): FibSwing | null {
  ...
  if (diff382 > 0.15) {
    ...
    return {
      ...
      fibKey,
      focusBars: Math.min(N - loIdx + 55, N),
    }
  }
...
function buildStepsFromSwing(swing: FibSwing): TutorialStep[] {
  const { lowDate, lowPrice, highDate, highPrice,
          bounceDate, bouncePrice, fibKey, focusBars } = swing
  ...
  return [
    ...
    /* STEP 5  퀴즈 */
    {
      ...
      completionMessage: correctFeedbacks[fibKey],
    },
